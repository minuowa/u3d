...
public class TweenHelper
{
    ...
    private static EasingFunction[] easeList = new EasingFunction[(int)EaseType.punch];
    public static float Update(EaseType easeType, float start, float end, float value)
    {
        EasingFunction ease=easeList[(int)easeType];
        if (ease == null)
        {
            switch (easeType)
            {
